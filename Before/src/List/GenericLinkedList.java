...
//		
//		public Node(T data, Node next) {//자료구조를 구현하기 위해서 사용된 데이터 타입! 내부적인 데이터를 어떻게 다루는지..내부적 자료의 모델링이 node
//			this.data = data;
//			this.next = next;
//		}
//	
...
//			prev.next = newNode; 
//			if(prev == this.tail) { //꼬리에 넣을 경우. newnode가 꼬리가 되어야한다.  index>= count가 아닌 index > count이므로. index>= count이면 꼬리에 insert못함..
//			//(next == null)위와 같은 의미 
//				this.tail = newNode;
//			}
//		}
...
//			if(next == null) { // 앞도 널이고 뒤도 널이라는것은 노드가 하나라는것
//				if (next.next == null) {
